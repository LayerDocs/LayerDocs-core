...
testMatrix(
    ...
    async (page, docType) => {
        ...
        await expect(body).toHaveClass(/layerdocs/);
        await expect(body).toHaveClass(new RegExp(`layerdocs-${docType}`));

        const bgColor = await getComputedColor(page, "var(--ld-background-color)");

        if (docType === "paged") {
            // Paged: body has different background, pages have --ld-background-color
            ...
            // Non-paged: body has --ld-background-color
