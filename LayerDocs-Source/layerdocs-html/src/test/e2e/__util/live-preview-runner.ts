 ...
 * the `.layerdocs` element must be visible and `isReady()` must return true.
 */
async function waitForIframeReady(page: Page, timeout: number): Promise<void> {
    ...
    // Wait for .layerdocs to be visible inside the iframe.
    const remaining = Math.max(deadline - Date.now(), 1000);
    await expect(frameLocator.locator(".layerdocs")).toBeVisible({timeout: remaining});
