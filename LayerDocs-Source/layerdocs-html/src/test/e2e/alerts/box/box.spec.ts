...
test("renders box types with correct colors and icons", async (page) => {
    ...
    for (const type of BOX_TYPES) {
        ...
        const titleFgColor = await getComputedColor(page, `var(--ld-${type}-title-foreground-color)`);
        const contentFgColor = await getComputedColor(page, `var(--ld-${type}-content-foreground-color)`);
        const bgColor = await getComputedColor(page, `var(--ld-${type}-background-color)`);
