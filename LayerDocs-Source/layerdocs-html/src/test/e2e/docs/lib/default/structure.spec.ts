...
for (const {subpath, title, heading} of pages) {
    ...
    test(`${pageName}: docs body is visible with correct margins`, async (page) => {
        // Body should have layerdocs-docs class and be visible
        const body = page.locator("body.layerdocs-docs");
        ...
        const leftAside = page.locator(".layerdocs-docs > .content-wrapper > aside:first-child");
        ...
        const rightAside = page.locator(".layerdocs-docs > .content-wrapper > aside:last-child");
    ...
    test(`${pageName}: has correct title and heading`, async (page) => {
        ...
        const h1 = page.locator(".layerdocs-docs > .content-wrapper > main h1");
        await expect(h1).toHaveCount(1);
        await expect(h1).toHaveText(title);

        // Check h2 heading in main content
        const h2 = page.locator(".layerdocs-docs > .content-wrapper > main h2");
