...
testMatrix(
    ...
    ["plain", "paged", "slides"],
    async (page) => {
        const mainFont = await getCssVar(page, "--ld-main-font");
        const headingFont = await getCssVar(page, "--ld-heading-font");
        const boxHeadingFont = await getCssVar(page, "--ld-box-heading-font");
        const codeFont = await getCssVar(page, "--ld-code-font");
...
testMatrix(
    ...
    async (page, docType) => {
        const mainFontSize = await getComputedSizeProperty(page, "var(--ld-main-font-size)");
        const codeSpanFontSize = await getComputedSizeProperty(page, "var(--ld-code-span-font-size)");
        const codeBlockFontSize = await getComputedSizeProperty(page, "var(--ld-code-block-font-size)");
        const captionFontSize = await getComputedSizeProperty(page, "var(--ld-caption-font-size)");
        ...
        const slidesCodeBlockFontSize = await getComputedSizeProperty(codeBlockParent, "var(--ld-slides-code-block-font-size)");
        const codeBlockStyle = await evaluateComputedStyle(codeBlock);
...
testMatrix(
    ...
    async (page, docType) => {
        ...
        const smCodeBlockFontSize = await getComputedSizeProperty(codeBlockParent, "var(--ld-sm-code-block-font-size)");
        ...
        const tableCaptionStyle = await evaluateComputedStyle(tableCaption);
        const figCaptionStyle = await evaluateComputedStyle(figCaption);
        const smCaptionFontSize = await getComputedSizeProperty(page, "var(--ld-sm-caption-font-size)");
