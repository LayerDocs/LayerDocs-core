...
testMatrix(
    ...
    async (page) => {
        ...
        const lineColor = await getComputedColor(page, "var(--ld-mermaid-node-line-color)");
        const bgColor = await getComputedColor(page, "var(--ld-mermaid-node-background-color)");

        // Legend text must have fill color --ld-mermaid-node-line-color
        const legendText = svg.locator(".legend text").first();
        await expect(legendText).toHaveCSS("fill", lineColor);

        // No path or rect should have fill color --ld-mermaid-node-background-color
