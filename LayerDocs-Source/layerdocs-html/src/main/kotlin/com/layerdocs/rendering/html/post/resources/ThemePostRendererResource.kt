 ...
 * `assembleThemes` Gradle task. In a LayerDocs distribution it ends up under `lib/html/theme/`.
 *
 * Expected layout:
 * ```
 * <themeDirectory>/
            ...
            if (warnIfMissing) {
                Log.error(
                    "'${kindDirectory.name}' theme not found: $name (looked in ${directory.file.absolutePath}).\n" +
                        "For a list of available themes, check https://layerdocs.com/wiki/themes or your local theme directory.",
                )
            }
            return null
        }
