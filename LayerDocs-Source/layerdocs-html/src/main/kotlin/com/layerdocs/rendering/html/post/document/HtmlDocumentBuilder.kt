...
import com.layerdocs.rendering.html.post.resources.HTML_LIBRARY_OUTPUT_PATH
import com.layerdocs.rendering.html.post.resources.HTML_SCRIPT_FILE_NAME
import com.layerdocs.rendering.html.post.resources.HTML_SCRIPT_OUTPUT_PATH
import com.layerdocs.rendering.html.post.thirdparty.HeadContribution
import com.layerdocs.rendering.html.post.thirdparty.ThirdPartyLibrary
...
class HtmlDocumentBuilder(
    ...
    private fun HEAD.documentMetadata() {
        meta(name = "generator", content = "LayerDocs")
        meta(charset = "UTF-8")
    ...
    /**
     * Emits a `<link rel="canonical">` tag if a [base URL][com.layerdocs.core.context.options.HtmlOptions.baseUrl]
    ...
    /** Emits LayerDocs-specific meta tags for root path resolution and, for docs, the search index. */
    private fun HEAD.layerdocsMeta() {
        meta(name = "layerdocs:root-path", content = relativePathToRoot)
        if (document.type == DocumentType.DOCS) {
            meta(name = "layerdocs:search-index", content = "$relativePathToRoot/search-index.json")
        }
    }

    /** Loads the main LayerDocs script and initializes the capabilities object. */
    private fun HEAD.layerdocsScript() {
        script(src = "$relativePathToRoot/$HTML_SCRIPT_OUTPUT_PATH/$HTML_SCRIPT_FILE_NAME") {}
        script { unsafe { raw("const capabilities = window.layerdocsCapabilities") } }
    }
     ...
     * which serves as the single source of truth shared with [ThirdPartyPostRendererResource][com.layerdocs.rendering.html.post.resources.ThirdPartyPostRendererResource].
     */
