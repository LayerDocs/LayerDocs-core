 ...
 */
export abstract class PageMarginsDocumentHandler extends DocumentHandler<PagedLikeLayerDocsDocument<any>> {
     ...
     */
    private selectPageMarginInitializers(page: LayerDocsPage | HTMLElement): HTMLElement[] {
    ...
    async onPostRendering() {
        ...
        this.layerdocsDocument.getPages().forEach(page => {
     ...
     */
    private getMarginPositionName(initializer: HTMLElement, page: LayerDocsPage): string | null {
        const pageType = this.layerdocsDocument.getPageType(page);
     ...
     * @param marginPositionName The margin position name (e.g., "top-left", "bottom-center")
     */
    abstract apply(initializer: HTMLElement, page: LayerDocsPage, marginPositionName: string): void;
}