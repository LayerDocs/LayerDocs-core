...
import {PagedLikeLayerDocsDocument, LayerDocsPage} from "../paged-like-layerdocs-document";
 ...
 */
export class PageNumbers extends DocumentHandler<PagedLikeLayerDocsDocument<any>> {
    /**
     ...
     */
    private getCurrentPageNumberElements(page: LayerDocsPage): NodeListOf<HTMLElement> {
     ...
     */
    private getPageNumberResetMarkers(page: LayerDocsPage): HTMLElement[] {
     ...
     */
    private getPageNumberFormatMarkers(page: LayerDocsPage): HTMLElement[] {
     ...
     */
    private updateTotalPageNumbers(pages: LayerDocsPage[]) {
     ...
     */
    private updateCurrentPageNumbers(pages: LayerDocsPage[]) {
        let pageNumber = 1;
        let currentFormat = "1";
        pages.forEach(page => {
            ...
            this.layerdocsDocument.setDisplayPageNumber(page, formattedPageNumber);
    ...
    private updateTableOfContentsPageNumbers() {
        ...
        tocs.forEach(nav => {
            nav.querySelectorAll<HTMLAnchorElement>(':scope a[href^="#"]').forEach(anchor => {
                ...
                const displayNumber = target ? this.layerdocsDocument.getDisplayPageNumber(this.layerdocsDocument.getPage(target)) : undefined;
    ...
    async onPostRendering() {
        const pages = this.layerdocsDocument.getPages();
