...
class DummyDocument implements PagedLikeLayerDocsDocument<LayerDocsPage> {
  constructor(private readonly pages: LayerDocsPage[] = [], private readonly types: Array<'left'|'right'> = []) {
  }

  getPages(): LayerDocsPage[] {
    return this.pages;
  }

  getPageNumber(_page: LayerDocsPage): number {
    return 1;
  }

  getDisplayPageNumber(_page: LayerDocsPage): string {
    return "1";
  }

  getPageType(page: LayerDocsPage): 'left' | 'right' {
    const index = this.pages.indexOf(page);
    return this.types[index] ?? 'right';
  }

  getPage(_element: HTMLElement): LayerDocsPage | undefined {
    return undefined;
  }

  setDisplayPageNumber(_page: LayerDocsPage, _pageNumber: string): void {
  }
...
class RecordingHandler extends PageMarginsDocumentHandler {
  ...
  apply(initializer: HTMLElement, page: LayerDocsPage, marginPositionName: string) {
    this.applied.push({
      pageIndex: (this.layerdocsDocument.getPages().indexOf(page)),
...
const createPage = (initializers: HTMLElement[] = []): LayerDocsPage => {
