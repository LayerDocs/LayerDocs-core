 ...
 */
export type SlidesPage = { slide: HTMLElement, background: HTMLElement } & LayerDocsPage;

/**
 * Slides document implementation for Reveal.js presentations.
 */
export class SlidesDocument implements PagedLikeLayerDocsDocument<SlidesPage> {
    /**
     * Retrieves a configuration property from the global configuration (`slidesConfig`).
     * Configuration is injected by LayerDocs's `.slides` function.
     */
