package com.layerdocs.installlayout

import com.layerdocs.core.log.Log
import java.io.File

/**
 * Resolves the LayerDocs install `lib/` directory and wraps it as an [InstallLayout].
 *
...
internal object InstallDirectoryResolver {
    /** Name of the install `lib/` directory in a LayerDocs distribution. */
    ...
    fun resolve(): InstallLayout {
        val executable =
            thisExecutableFile
                ?: error("Cannot determine the LayerDocs executable location. The JAR's code source may be unavailable.")
    ...
    private fun resolveFrom(executable: File): File {
        ...
        error(
            """
            Cannot resolve the LayerDocs install directory.
