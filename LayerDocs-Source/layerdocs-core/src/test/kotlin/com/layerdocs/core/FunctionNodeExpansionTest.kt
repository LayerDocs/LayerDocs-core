...
import com.layerdocs.core.ast.attributes.error.asNode
...
import com.layerdocs.core.function.call.FunctionCallNodeExpander
import com.layerdocs.core.function.library.LibraryRegistrant
...
/**
 * Tests for functions called from a LayerDocs source.
...
class FunctionNodeExpansionTest {
    ...
    fun `resource content expansion as inline`() {
        ...
        assertEquals(1, node.children.size)
        assertNodeEquals(Text("Hello LayerDocs!"), node.children.first())
    }
    ...
    fun `resource content expansion as block`() {
        ...
        assertNodeEquals(Paragraph(listOf(Text("Hello LayerDocs!"))), node.children.first())
    }
