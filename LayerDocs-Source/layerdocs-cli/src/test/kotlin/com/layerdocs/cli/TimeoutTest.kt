...
import com.layerdocs.cli.exec.ExecutionTimeoutException
...
class TimeoutTest {
    ...
    @Test
    fun `invokes onTimeout handler instead of throwing`() {
        val result =
            runWithTimeout(1, onTimeout = { e -> "timed out after ${e.timeoutSeconds}" }) {
                Thread.sleep(5000)
                "completed"
            }
        assertEquals("timed out after 1", result)
    }
