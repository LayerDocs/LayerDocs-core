...
import com.layerdocs.stdlib.ContextSandbox
...
private fun forSandboxes(
    ...
    sandboxes.forEach {
        Log.info("Testing ecosystem success in sandbox: ${it.layerdocsName}")
        block(it.layerdocsName)
    }

    if (expectOthersFail) {
        val others = ContextSandbox.entries.toSet() - sandboxes.toSet()
        others.forEach { sandbox ->
            Log.info("Testing ecosystem failure in sandbox: ${sandbox.layerdocsName}")
            assertFails {
                block(sandbox.layerdocsName)
            }
        }
    }
}
