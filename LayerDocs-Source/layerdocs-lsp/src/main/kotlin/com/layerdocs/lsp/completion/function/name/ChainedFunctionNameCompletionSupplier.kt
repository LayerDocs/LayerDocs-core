...
import com.layerdocs.lsp.tokenizer.findMatchingTokenBeforeIndex
import com.layerdocs.lsp.tokenizer.getTokenAtSourceIndex
import com.layerdocs.lsp.util.remainderUntilIndex
...
class ChainedFunctionNameCompletionSupplier(
    ...
    override fun transformIndex(
        cursorIndex: Int,
        text: String,
    ) = cursorIndex - LayerDocsPatterns.FunctionCall.CHAIN_SEPARATOR.length
    ...
    private fun isCompletableCall(
        ...
        if (call.remainderUntilIndex(index) == LayerDocsPatterns.FunctionCall.CHAIN_SEPARATOR) {
            return true
        }
